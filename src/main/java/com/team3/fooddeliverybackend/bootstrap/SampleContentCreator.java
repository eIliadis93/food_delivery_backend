...
public class SampleContentCreator extends BaseComponent implements CommandLineRunner {
    ...
    public void run(String... args) throws Exception {
        Account newAccount = accountService.create(Account.builder().email("guest1@gmail.com").password("admin1234!")
                .firstname("Joey").lastname("Tribbiani").age(30).address("Somewhere in Brooklyn").build());
        logger.info("Created {}.", newAccount);
    ...
    List<Product> products = List.of(
      ...
      Product.builder().name("Ice Cream").serial("asdasdasd").price(BigDecimal.valueOf(2.80)).productCategory(ProductCategory.DESERT).build(),
      Product.builder().name("Margarita").serial("asdasdasd").price(BigDecimal.valueOf(8)).productCategory(ProductCategory.DRINK).build(),
      Product.builder().name("Spaghetti").serial("asdasdasd").price(BigDecimal.valueOf(6.30)).productCategory(ProductCategory.FOOD).build()
    );
